...
export default function CategoryPage() {
  ...
  /* Get the last 8 articles and last 8 advices from the store */
  /* TODO: Install a pagination system */
        ...
        <Grid container spacing={2}>
          {advices.map((advice) => (
            <Grid item xs={12} md={6} key={`${advice.id}`}>
              <AdvicesMediumCard advice={advice} userId={userId} />
            </Grid>
          ))}
        </Grid>
        <Box
...
function AdvicesMediumCard({ advice, userId }) {
  /* Truncate the content of the card */
  const TruncateContent = styled.div`
    overflow: hidden;
    overflow-wrap: break-word;
